...
- **Visual world** ("Field"): a blue field at the top holds each tab's headline (`heroes.ts` computes them from the dashboard), the counts, and the tabs; lists below are plain. Light is the default theme and dark is composed separately; the preference is persisted and applied as `data-theme` on the document root so portaled layers follow it. Course colors are slots resolved per theme (`theme.ts`). The direction contract lives in `apps/extension/.impeccable/surfaces/src-sidepanel.md`; the built system is recorded in `DESIGN.md`.
- **Session dismissals** (`src/sidepanel/dismissals.ts`): tasks, change events, and banners can be hidden for the current panel session. The sets live in React state only, feed `buildDashboard` so every count and view agrees, and reset when the snapshot's `capturedAt` changes or the student presses Refresh.
