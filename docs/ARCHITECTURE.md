...
- **Session dismissals** (`src/sidepanel/dismissals.ts`): tasks and banners can be hidden for the current panel session. The sets live in React state only, feed `buildDashboard` so every count and view agrees, and reset when the snapshot's `capturedAt` changes or the student presses Refresh.
