...
export const changeIcon = (event: ChangeEvent): { icon: React.ReactNode; tone: Tone } => {
  switch (event.kind) {
    ...
    case "due-date-changed":
      return { icon: <ArrowRightLeft size={15} aria-hidden="true" />, tone: "amber" };
    case "became-overdue":
    ...
    case "visibility-changed":
      return event.after?.visibility === "hidden"
        ? { icon: <EyeOff size={15} aria-hidden="true" />, tone: "amber" }
        : { icon: <Eye size={15} aria-hidden="true" />, tone: "blue" };
    case "item-removed":
      return { icon: <Trash2 size={15} aria-hidden="true" />, tone: "amber" };
