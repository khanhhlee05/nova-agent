...
export const suggestedQuestions = (dashboard: Dashboard): string[] => {
  ...
  const second = dashboard.counts.overdue > 0 ? "What is overdue?" : dashboard.counts.thisWeek > 0 ? "What is due this week?" : "What is due later?";
...
export const AskChips = ({ dashboard, disabled, onPick }: AskChipsProps) => (
  <div className="ask-chips" role="group" aria-label="Suggested questions">
    {suggestedQuestions(dashboard).map((question) => (
      <button key={question} type="button" className="chip-on-field" disabled={disabled} onClick={() => onPick(question)}>
        {question}
      </button>
    ))}
  </div>
